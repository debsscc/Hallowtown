...
func _input(event):
	...
	if Input.is_action_pressed("Tab") and Player.running == 1: # and not Player.animationCommit and not Inventory.DraggedItem.visible and not ShopMenu.visible:
		if visible:
			visible = false
		else:
			visible = true
	...
	elif visible and (Input.is_action_pressed("scroll_down")):
		if IndicatorSlot == 1:
			IndicatorSlot = 10
		else:
			IndicatorSlot -= 1
