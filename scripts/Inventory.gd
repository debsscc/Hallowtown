...
var stacked_items = {"StrawberrySeeds":10, "Gold":10}
...
func _input(event):
	...
	if Input.is_action_pressed("Q") and Player.running == 1: #and not Player.animationCommit and not DraggedItem.visible and not ShopMenu.visible:
		if visible:
			visible = false
		else:
			visible = true
		...
		if _contains(item): #if the inventory already has one of these items, update the label accordingly
			...
			for texture in textures_and_labels.keys():
				if texture.get_texture() == null:
					texture.set_texture(load("res://assets/Items/" + item + ".png"))
